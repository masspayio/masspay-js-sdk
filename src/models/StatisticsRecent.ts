/**
 * MassPay API
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: info@masspay.io
 *
 * NOTE: This file is auto generated.
 * Do not edit the file manually.
 */
export type StatisticsRecent = {
  /**
   * The ID of the object
   */
  id?: string;
  /**
   * Item tyep
   */
  type: StatisticsRecent.type;
  /**
   * Amount is charged without a decimal place.
   */
  amount: number;
  /**
   * Payment product string
   */
  payment_product?: StatisticsRecent.payment_product;
  /**
   * The date and time when the transaction was first created
   */
  created_at: string;
  /**
   * The organisation connected to this object
   */
  organisation?: {
    /**
     * The ID of the organisation object
     */
    id?: string;
    /**
     * The name of the organisation
     */
    name?: string;
  };
};
export namespace StatisticsRecent {
  /**
     *
    Item tyep
     */
  export enum type {
    TRANSACTION = 'transaction',
    BLOCKED = 'blocked',
    REFUND = 'refund',
  } /**
     *
    Payment product string
     */
  export enum payment_product {
    EMPTY = '',
    ALIPAY = 'alipay',
    APPLEPAY = 'applepay',
    BCMC = 'bcmc',
    BELFIUS = 'belfius',
    BITPAY = 'bitpay',
    BPWALLET = 'bpwallet',
    CARD = 'card',
    CARDEXTERNAL = 'cardexternal',
    CREDITCLICK = 'creditclick',
    DIRECTDEBIT = 'directdebit',
    DIRECTDEBITPLUS = 'directdebitplus',
    DUMMY = 'dummy',
    EPS = 'eps',
    ETRANSFER = 'etransfer',
    EZEEWALLET = 'ezeewallet',
    FINSHARK = 'finshark',
    GCASH = 'gcash',
    GIROPAY = 'giropay',
    GOOGLEPAY = 'googlepay',
    IDEAL = 'ideal',
    IMPS = 'imps',
    INTERAC = 'interac',
    MULTIBANCO = 'multibanco',
    MYBANK = 'mybank',
    OPENNODE = 'opennode',
    P24 = 'p24',
    PAYPAL = 'paypal',
    PAYTM = 'paytm',
    PAYU = 'payu',
    PAYSAFECARD = 'paysafecard',
    POS = 'pos',
    SEPA = 'sepa',
    SKRILL = 'skrill',
    SOFORT = 'sofort',
    TRIPLEA = 'triplea',
    TRUSTPAY = 'trustpay',
    TRUSTLY = 'trustly',
    UNIONPAY = 'unionpay',
    UPI = 'upi',
    VOLT = 'volt',
    WECHATPAY = 'wechatpay',
  }
}
